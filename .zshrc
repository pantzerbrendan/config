...
ssh_start() { confirm_wrapper --root /etc/init.d/ssh "start"; }
ssh_stop() { confirm_wrapper --root /etc/init.d/ssh "stop"; }
ssh_restart() { confirm_wrapper --root /etc/init.d/ssh "restart"; }
edhosts() { confirm_wrapper --root nano "/etc/hosts"; }
visudo() { confirm_wrapper --root nano "/etc/sudoers"; }
...
# Get weather (replace the 71822 in the url with your own zipcode, call it by typing weather)
 ...
 WEATHERARRAY=( `elinks -dump "http://www.google.com/search?hl=en&lr=&client=firefox-a&rls=org.mozilla%3Aen-US%3Aofficial&q=weather+21122&btnG=Search" | grep -A 5 -m 1 "Weather for" | grep -v "Add to "`)
 echo ${WEATHERARRAY[@]}
 }
...
## School
alias epitech='xdg-open https://intra.epitech.eu'
alias uqar='xdg-open https://portail.uqar.ca'
